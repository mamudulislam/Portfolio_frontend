    ...
    useEffect(() => {
        ...
        if (status === 'success') {
            timer = setTimeout(() => {
                setStatus('idle');
                setResponseMessage('');
            }, 600); // 1 minute = 60000 milliseconds
        }
                        ...
                        <form className="space-y-6" onSubmit={handleSubmit}>
                            ...
                            <div className="h-12">
                                {status === 'idle' && (
                                    <button 
                                        type="submit"
                                        className="w-full py-4 bg-gradient-to-r from-primary to-secondary text-white font-bold uppercase tracking-widest rounded-lg hover:opacity-90 transition-opacity flex items-center justify-center gap-2 shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                                    >
                                ...
                                {status === 'success' && (
                                    <div className="flex items-center justify-center w-full h-full text-green-500 bg-green-500/10 rounded-lg p-3">
                                        <CheckCircle className="mr-2 flex-shrink-0" size={20} />
                                        <span className="text-sm font-medium">{responseMessage}</span>
                                    </div>
                                )}
                                {status === 'error' && (
                                    <div className="flex items-center justify-center w-full h-full text-red-500 bg-red-500/10 rounded-lg p-3">
